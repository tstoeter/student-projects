	...
	public:
		...
		*/
		...
		/*
			...
			PARAMETERS:
				v	- Vertex list of triangle mesh, where the vertices and their three coordinates are stored consequtively, e.g., the STL vector contains: ax, ay, az, bx, by, bz, cx, cy, cz, ... for the vertices a, b, c, ...
		...
		*/
		...
		bool is_boundary_halfedge(int h);
		int halfedge_face(int h);

		vector<int> halfedges_around_vertex(int v);
		vector<int> vertices_around_vertex(int v);
		vector<int> faces_around_vertex(int v);

		vector<int> halfedges_of_face(int f);
		vector<int> faces_around_face(int f);
};
...
/*
	...
	RETURNS:
		...
		False, otherwise.
*/
bool heds::is_boundary_halfedge(int h)
{
...
*/
vector<int> heds::halfedges_around_vertex(int v)
{
...
*/
vector<int> heds::vertices_around_vertex(int v)
{
	assert(v >= 0 && v < num_vertices);
...
*/
vector<int> heds::faces_around_vertex(int v)
{
	assert(v >= 0 && v < num_vertices);
...
*/
vector<int> heds::halfedges_of_face(int f)
{
	vector<int> hes(3);
...
*/
vector<int> heds::faces_around_face(int f)
{
